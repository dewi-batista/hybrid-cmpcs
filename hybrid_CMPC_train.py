...
# hyperparameters
max_epochs = 2
n_components_train = 2 ** 13
n_chunks = 64        # if you run OOM, use n_chunks (e.g. n_chunks = 32)
seed_everything(42)
...
for latent_dim in [2, 4, 8, 16, 32]:
    for lamda in [0, 0.2, 0.4, 0.6, 0.8, 1]:
        ...
        model = ContinuousMixture(
            decoder=decoder,
            sampler=GaussianQMCSampler(latent_dim, n_components_train),
            lamda = lamda,
            X_num = 780,
            Y_num = 4,
        )
        ...
        # trainer set up
        model_path = f'/logs/{dset}/hybrid/latent_dim_{latent_dim}/num_bins_{n_components_train}/lambda_{lamda:.2f}'
